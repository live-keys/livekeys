...
CodePalette *LanguageQmlHandler::openBinding(QmlEditFragment *edit, QString paletteName){
    ...
    // check if duplicate
    PaletteContainer::PaletteInfo paletteInfo = d->projectHandler->paletteContainer()->findPaletteByName(paletteName);
    if ( edit->bindingPalette() ){
        if ( edit->bindingPalette()->path() == QString::fromStdString(paletteInfo.path().data()) )
            return edit->bindingPalette();
    }
    CodePalette* cp = edit->palette(QString::fromStdString(paletteInfo.path().data()));
    if ( cp ){
...
QJSValue LanguageQmlHandler::expand(QmlEditFragment *edit, const QJSValue &val){
    ...
    if ( val.hasProperty("palettes") ){
        ...
        while ( palettesIt.next() ){
            ...
            PaletteContainer::PaletteInfo paletteInfo = d->projectHandler->paletteContainer()->findPaletteByName(paletteName);
            if ( !paletteInfo.isValid() )
                return QJSValue();

            if ( edit->bindingPalette() && edit->bindingPalette()->path() == QString::fromStdString(paletteInfo.path().data()) ){
            ...
            if ( !PaletteLoader::configuresLayout(paletteInfo) ){
                ...
                return d->projectHandler->paletteContainer()->paletteContent(paletteInfo);
            }
        }
    }
    return QJSValue();
...
lv::CodePalette* LanguageQmlHandler::edit(lv::QmlEditFragment *edit){
    ...
    PaletteContainer::PaletteInfo pi = d->projectHandler->paletteContainer()->findFirstPalette("edit/qml");
    CodePalette* palette = d->projectHandler->paletteContainer()->createPalette(pi);
...
int LanguageQmlHandler::addObjectToCode(int position, const QJSValue &typeOptions, const QJSValue &properties){
    ...
    auto typeReference = QmlTypeReference::split(type);
    QString typeName = typeReference.name();
    QString importAs = "";
    if ( !typeReference.path().isEmpty() ){
        const DocumentQmlInfo::ImportList &imports = d->documentInfo()->imports();
        for ( auto it = imports.begin(); it != imports.end(); ++it ){
            if ( it->uri() == typeReference.path() ){
                importAs = it->as();
                break;
            }
        }
    ...
    if ( tbStart == tbEnd ){ // inline object declaration
        ...
        insertionText += "} ";
        cursorPosition = insertionPosition + typeName.size() + 3;
    } else { // multiline object declaration
        QString indent = getBlockIndent(tbStart) + "    ";
        insertionPosition = tbEnd.position();
        QTextBlock tbIt = tbEnd.previous();
        ...
        insertionText = indent + (importAs.isEmpty() ? "" : (importAs + ".")) + typeName + "{\n";
        if (id != "") {
        ...
        cursorPosition = insertionPosition + indent.size() + typeName.size() + 1;
    }
    ...
    return cursorPosition - 1 - typeName.size();
}
...
QJSValue LanguageQmlHandler::findPalettesForDeclaration(QmlDeclaration::Ptr declaration)
{
    ...
    std::list<PaletteContainer::PaletteInfo> result = d->projectHandler->paletteContainer()->findPalettes(declaration->type().join());
    ...
    if (declaration->isForComponent()){
        ...
        if ( declaration->isForList() ){
            ...
            result.insert(result.end(), prop.begin(), prop.end());
        }
    }
